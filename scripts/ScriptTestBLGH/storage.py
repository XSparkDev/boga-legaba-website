...
def sync_to_supabase(data: dict, window_from: str, window_to: str) -> dict[str, int]:
    ...
    try:
        ...
        sb.table("property").upsert({
            ...
            "raw": bb,
            "roomtypemode": data.get("roomtypemode"),
            "booking_url": f"https://book.nightsbridge.com/{bbid}",
        ...
        if isinstance(roomtypes, dict):
            for bbrtid, rt in roomtypes.items():
                sb.table("room_type").upsert({
                    ...
                    "room_size_sqm": rt.get("roomsizesqm"),
                    "rate_scheme": rt.get("ratescheme"),
                    "ota_room_type_code": rt.get("otaroomtypecode"),
                    "type_order_by": rt.get("orderby"),
                    "raw": rt,
                    "updated_at": datetime.utcnow().isoformat(),
                }).execute()
        ...
        # Closeouts — replace window entries for synced rooms
        sb.table("closeout").delete().in_("bbroomid", room_ids).execute()
        ...
        # Calendar events
        sb.table("calendar_event").delete().eq("bbid", bbid).execute()
        for ev in data.get("events") or []:
            sb.table("calendar_event").insert({
                "bbid": bbid,
                "event_date": ev.get("date") or ev.get("eventdate") or ev.get("fromdate"),
        ...
        # Audit snapshot of payload shape
        sample_room = (data.get("rooms") or [{}])[0] if data.get("rooms") else None
        sample_booking = (data.get("bookings") or [{}])[0] if data.get("bookings") else None
        sb.table("nb_api_snapshot").insert({
            "sync_run_id": sync_id,
            "message_name": "BookingCalendarRQ",
            "window_from": window_from,
            "window_to": window_to,
            "payload_keys": list(data.keys()),
            "room_count": len(data.get("rooms") or []),
            "roomtype_count": len(roomtypes) if isinstance(roomtypes, dict) else 0,
            "booking_count": len(data.get("bookings") or []),
            "closeout_count": len(data.get("closeouts") or []),
            "event_count": len(data.get("events") or []),
            "payload_sample": {
                "bb": bb,
                "sample_room": sample_room,
                "sample_roomtype": next(iter(roomtypes.values())) if isinstance(roomtypes, dict) and roomtypes else None,
                "sample_booking_keys": list(sample_booking.keys()) if isinstance(sample_booking, dict) else None,
                "sample_stay_keys": list((sample_booking.get("rooms") or [{}])[0].keys()) if isinstance(sample_booking, dict) else None,
            },
