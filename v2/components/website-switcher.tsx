    ...
    >
      ...
      >
        {WEBSITE_OPTIONS.map((site) => {
          const active = site.id === CURRENT_SITE_ID
          const href = getSwitchTargetUrl(pathname, site.id)
  ...
  return (
    <div className={cn("border-b pb-4", isDark ? "border-white/10" : "border-white/15")}>
      ...
      <div className="mt-3 flex flex-col gap-2">
        {WEBSITE_OPTIONS.map((site) => {
          const active = site.id === CURRENT_SITE_ID
          const href = getSwitchTargetUrl(pathname, site.id as SiteId)
