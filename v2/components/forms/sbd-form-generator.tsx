...
export function SbdFormGenerator({ formData }: { formData: SbdFormData }) {
  ...
  return (
    <div id="sbd-form-generator" className="rounded-xl border border-warm-sand/70 bg-cream px-5 py-5">
      <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-body-brown">
        Government per diem · SBD forms
      </p>
      <p className="mt-2 text-sm leading-relaxed text-muted-brown">
        Generate the standard procurement forms pre-filled with your enquiry details. Review, sign and
        return with your booking.
      </p>
      ...
      <div className="mt-4 space-y-3">
        {sbdForms.map((sbd) => (
          <div
            key={sbd.id}
            className="flex flex-col gap-3 rounded-xl border border-warm-sand/60 bg-off-white p-4 sm:flex-row sm:items-center sm:justify-between"
          >
            <div>
              <p className="flex items-center gap-2 font-sans text-sm font-medium text-deep-earth">
                <FileText className="h-4 w-4" />
                {sbd.title}
              </p>
              <p className="mt-1 text-xs text-muted-brown">{sbd.summary}</p>
            </div>
            <button
              type="button"
              disabled={!SBD_FORMS_ENABLED || !ready}
              onClick={() => openForm(sbd.id, sbd.title)}
              className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-deep-earth bg-deep-earth px-4 py-2 font-sans text-xs font-medium text-white transition-colors hover:bg-body-brown disabled:cursor-not-allowed disabled:opacity-40"
            >
      ...
      {!ready ? (
        <p className="mt-3 text-xs text-muted-brown">
